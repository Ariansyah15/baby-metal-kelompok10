...
class Booking extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($dtb < 1) {
            $this->session->set_flashdata(
                'pesan',
                '<div class="alert alertmassege alert-danger" role="alert">Tidak Ada Buku dikeranjang</div>'
            );
            redirect(base_url());
        } else {
    ...
    public function tambahBooking()
    {
        ...
        if ($databooking > 0) {
            $this->session->set_flashdata(
                'pesan',
                '<div class="alert alertdanger alertmessage" role="alert">Masih Ada booking buku sebelumnya yang belum diambil.<br> Ambil Buku yang dibooking atau tunggu 1x24 Jam untuk bisa booking kembali </div>'
            );
            redirect(base_url());
        }
        ...
        if ($temp > 0) {
            $this->session->set_flashdata(
                'pesan',
                '<div class="alert alertdanger alert-message" role="alert">Buku ini Sudah anda booking </div>'
            );
        ...
        if ($tempuser == 3) {
            $this->session->set_flashdata(
                'pesan',
                '<div class="alert alertdanger alert-message" role="alert">Booking Buku Tidak Boleh Lebih dari 3</div>'
            );
            redirect(base_url() . 'home');
        }
        ...
        $this->session->set_flashdata(
            'pesan',
            '<div class="alert alertsuccess alertmessage" role="alert">Buku berhasil ditambahkan ke keranjang </div>'
        );
        redirect(base_url() . 'home');
    }
    ...
    public function bookingSelesai($where)
    {
        ...
        $isibooking = [
            'id_booking' => $this->ModelBooking->kodeOtomatis(
                'booking',
                'id_booking'
            ),
            ...
            'batas_ambil' => date(
                'Y-md',
                strtotime('+2 days', strtotime($tglsekarang))
            ),
            'id_user' => $where,
        ];
